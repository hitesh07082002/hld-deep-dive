...
How to approach it: Start by explaining what a hot key is — a single cache key receiving disproportionate traffic, like a trending topic or viral product. Then discuss solutions in order of complexity: (1) local in-memory cache on each app server as an L1 cache, reducing Redis hits by the number of servers, (2) key replication — store the same key under multiple names (product:123:v1, product:123:v2) and randomly choose one, spreading load across shards, (3) consistent hashing with read replicas on the hot shard. Discuss tradeoffs of each: L1 cache is simple but increases memory per server; key replication reduces hot-key load but complicates invalidation.

**Q: How would you ensure cache consistency with the database?**
How to approach it: Don't just say "use TTL." Walk through cache-aside with delete-on-write as the baseline. Explain why you delete rather than update (avoids race conditions where two concurrent writes leave stale data). Discuss the brief inconsistency window and whether it matters for this specific system. If strong consistency is needed, discuss write-through or distributed invalidation via events. Mention the tradeoff: stronger consistency means higher write latency and more complexity.

**Q: Your cache goes down. What happens and how do you handle it?**
How to approach it: Describe the immediate impact — all requests hit the database, which may not be sized for full traffic. Discuss mitigation: circuit breaker to shed load rather than crashing the database, graceful degradation (serve stale data from a backup cache or reduce feature set), database connection pooling to prevent connection exhaustion. Long-term: use Redis Cluster or Sentinel for high availability with automatic failover, and maintain hot standby replicas. Discuss cache warming strategies for when the new cache instance comes up.

**Q: When would you NOT use caching?**
How to approach it: Show judgment by discussing specific scenarios. Write-heavy systems with low read frequency — caching adds overhead without benefit. Data that must be absolutely real-time consistent (financial transactions). Systems where the working set is larger than available cache memory, leading to constant eviction and low hit rates. Small-scale systems where the database comfortably handles the load — caching adds operational complexity for no gain.

---

## Connections to Other Concepts

**Concept 11 - Consistent Hashing**: When scaling a cache beyond a single node, consistent hashing determines which node stores which keys. Without consistent hashing, adding or removing a cache node would invalidate nearly every key (since traditional modular hashing remaps most keys). Consistent hashing ensures only ~1/N of keys are remapped when a node changes, which is critical for maintaining high hit rates during scaling events.

**Concept 08 - Database Replication**: Caching and replication are complementary read-scaling strategies. Replication creates copies of your database to handle more read traffic; caching keeps hot data in faster storage. A common architecture uses both: a Redis cache absorbs 85-95% of reads, and database read replicas handle the remaining cache misses. This combination can scale reads by 100x compared to a single database.

**Concept 03 - CDN & Edge Computing**: A CDN is essentially a geographically distributed cache for static content. Many of the same principles apply — TTL-based expiration, cache invalidation challenges, and cache hit ratio optimization. The difference is that CDN caching optimizes for network latency (serving content from a nearby edge), while application-level caching (Redis) optimizes for compute and database latency.

**Concept 14 - Message Queues & Stream Processing**: Event-driven cache invalidation often relies on message queues. When a write occurs, the service publishes an event to Kafka or Redis Pub/Sub, and cache invalidation workers consume these events to delete or update the relevant cache keys. This decouples the write path from cache management and enables cross-service cache invalidation in microservices architectures.

**Concept 19 - Fault Tolerance Patterns**: Cache failures are a common trigger for cascading failures. The circuit breaker pattern is essential for caching — if Redis becomes unreachable, the circuit breaker trips and routes requests directly to the database with appropriate rate limiting, instead of letting every request timeout against the dead cache and then also hit the database. Without a circuit breaker, a cache failure can easily become a full system outage.
